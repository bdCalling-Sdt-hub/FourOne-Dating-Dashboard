...
import { useGetApprovedUsersQuery } from "../../redux/features/users/approvedUsersList";
...
const ApprovedUsers = () => {
  ...
  const allUsers = data?.data?.attributes?.results;
  ...
  return (
    <div className="p-5 rounded-md mt-5">
      ...
      <div className="md:flex justify-between mb-5 items-center">
        <h2 className="text-xl sm:text-2xl font-semibold text-[#1a1a1a]">All Users</h2>
            ...
            {filteredUsers?.map((user, index) => (
              <tr key={user.id} className="border-b hover:bg-gray-100">
                ...
                <td className="px-4 py-3 flex items-center gap-2">
                  <img src={url + user.profileImage} alt="User" className="w-10 h-10 rounded-full" />
      ...
      <Modal open={isModalOpen} footer={null} onCancel={closeModal} centered>
        {selectedUser && (
          <>
            ...
            <p className="py-3 flex items-center justify-between">
              <strong>Joined:</strong> {moment(selectedUser.createdAt).format("DD MMMM, YYYY")}
            </p>
